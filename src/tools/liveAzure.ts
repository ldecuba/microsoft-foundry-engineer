...
function summarizeOk(value: unknown) {
  const result = value as { ok?: boolean; status?: string } | undefined;
  if (!result) {
    return "not-run";
  }
...
function getSingleAppInsightsName(result: unknown) {
  const data = (result as { data?: Array<{ name?: string }> } | undefined)?.data;
...
export async function generateReleaseEvidencePack(args: {
  ...
  appInsightsApp?: string;
  deploymentName?: string;
  includeSmokePrompt?: boolean;
  smokePrompt?: string;
  euUsersOrMarket?: boolean;
  interactsWithPeople?: boolean;
  generatesOrAltersContent?: boolean;
  biometricUse?: boolean;
  employmentEducationCreditHealthLawMigrationJusticeUse?: boolean;
  manipulativeOrExploitativeRisk?: boolean;
  providerOfGpaiModel?: boolean;
  usesThirdPartyGpaiModel?: boolean;
}) {
  ...
  const euAiAct = checkEuAiAct({
    ...
    employmentEducationCreditHealthLawMigrationJusticeUse:
      args.employmentEducationCreditHealthLawMigrationJusticeUse ?? false,
  ...
  const markdown = [
    ...
    markdownSection(
      "Executive Status",
      checks.map((check) => `- ${check.title}: ${check.status}`)
    ),
    ...
    markdownSection(
      "EU AI Act Evidence",
      euAiAct.findings.map((finding) => `- ${finding.severity}: ${finding.title} - ${finding.nextAction}`)
    ),
