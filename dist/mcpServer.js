...
export function createFoundryEngineerMcpServer() {
    ...
    server.tool("foundry_live_check_quota", "List Cognitive Services quota and usage for a region.", {
        location: z.string().default("westeurope")
    }, async (args) => jsonText(await listCognitiveServicesUsage(args)));
    server.tool("foundry_live_check_network_posture", "Inspect public network access, firewall rules, and private endpoint states for a Foundry or Azure AI Services account.", {
        accountName: z.string(),
        resourceGroup: z.string()
    }, async (args) => jsonText(await checkNetworkPosture(args)));
    server.tool("foundry_live_list_private_endpoints", "List private endpoints in a resource group, optionally filtered to one Foundry or Azure AI Services account.", {
        resourceGroup: z.string(),
    ...
    server.tool("foundry_live_list_app_insights", "List Application Insights components visible to the active Azure CLI login.", {
        resourceGroup: z.string().optional()
    }, async (args) => jsonText(await listAppInsightsComponents(args)));
    server.tool("foundry_live_query_app_insights_traces", "Query recent Application Insights traces, requests, and exceptions.", {
        app: z.string(),
        resourceGroup: z.string().optional(),
